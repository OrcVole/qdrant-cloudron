...
#
# Usage:
#   CLOUDRON_SERVER=my.example.com SRC_APP=qdrant.example.com NEW_APP=qdrant-restore.example.com ./backup-restore.sh
...
key_of() { cloudron --server "$CLOUDRON_SERVER" exec --app "$1" -- sh -c '. /app/data/.secrets/keys.env && printf %s "$QDRANT_ADMIN_API_KEY"' 2>/dev/null; }
...
cloudron --server "$CLOUDRON_SERVER" backup create --app "$SRC"

echo "== clone ${SRC} (latest backup) into fresh ${NEW} =="
cloudron --server "$CLOUDRON_SERVER" clone --app "$SRC" --backup latest --location "$NEW"
...
cloudron --server "$CLOUDRON_SERVER" exec --app "$NEW" -- grep -n 'max_resident_memory_percent:' /app/data/config/production.yaml
curl -s -o /dev/null -w '  restored app GET /healthz -> %{http_code}\n' "https://${NEW}/healthz"

if [ "${KEEP:-0}" != "1" ]; then
  echo "== cleanup: uninstall ${NEW} =="
  cloudron --server "$CLOUDRON_SERVER" uninstall --app "$NEW"
fi
