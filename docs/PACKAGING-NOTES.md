...
## Admin notes (postInstallMessage) formatting

Reviewed how the post-install note renders as Cloudron's "Admin notes" panel on the app Info page. The obvious markdown (a top-level `#` H1 like "Qdrant is installed", then several `##` sections) renders as a heavy wall of large text that duplicates the app name (already shown) and the red checklist boxes. The package now opens with one intro line (no H1), uses `###` headings, numbers the three critical steps, and keeps the prose short, leading with the generated credential and leaving depth to the README. The `checklist` items are kept to one or two sentences and complementary to the notes rather than a copy of them.

## minBoxVersion and iconUrl (caught by the stranger-path gate)
